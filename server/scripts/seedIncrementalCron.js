...
async function run() {
  try {
    ...
    let batchSize = 20; // Default users to seed per invocation
    let targetTotal = 1500; // Target total users in DB
    ...
    for (const arg of args) {
      if (arg.startsWith("--batch=")) {
        ...
        const parsed = parseInt(arg.split("=")[1], 10);
        if (!isNaN(parsed) && parsed > 0) targetTotal = parsed;
    ...
    console.log("================================================================");
    console.log(`🛰️  INCREMENTAL CRON SEEDER STARTING`);
    console.log("================================================================");
    ...
    const existingUsers = await User.countDocuments({ isAliasAccount: { $ne: true } });
    console.log(`📊 Current Active Users in DB : ${existingUsers}`);
    console.log(`🎯 Seeding Target (Total)     : ${targetTotal}`);

    if (existingUsers >= targetTotal) {
      console.log("================================================================");
      console.log(`✅ Target of ${targetTotal} users already reached. Seeding skipped!`);
      console.log("================================================================");
      await mongoose.disconnect();
      process.exit(0);
    }

    const remainingToSeed = targetTotal - existingUsers;
    const toSeed = Math.min(batchSize, remainingToSeed);

    console.log(`🚀 Incremental Run: Seeding ${toSeed} new users in this batch...`);
    console.log("================================================================");
    ...
    for (let i = 1; i <= toSeed; i++) {
      ...
      while (retryCount < maxRetries && !success) {
        ...
        try {
          ...
          console.log(`\n🌱 [${i}/${toSeed}] Registering: ${fullName} (${memberId}) | Sponsor: ${sponsorUser.memberId}`);
          ...
          const depositDocs = await DepositModel.create([
            {
              ...
              reviewReason: "Automated incremental seeding",
          ...
          // 2. Trigger standard autopool activation
          ...
          console.log(`   ✨ Stabilized. Placed ${queueResult?.placedCount || 0} nodes.`);
    ...
    const currentTotal = await User.countDocuments({ isAliasAccount: { $ne: true } });

    console.log("\n================================================================");
    console.log(`🎉 BATCH COMPLETED SUCCESSFULLY!`);
    console.log(`   Seeded in this batch : ${toSeed}`);
    console.log(`   Total Users in DB now: ${currentTotal} / ${targetTotal}`);
    console.log("================================================================");
