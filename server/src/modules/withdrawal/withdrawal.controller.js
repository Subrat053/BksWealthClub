...
export const createWithdrawalController = asyncHandler(async (req, res) => {
  const data = await withdrawalService.createRequest({ userId: req.user.sub, payload: req.body });
...
export const getWithdrawalHistoryController = asyncHandler(async (req, res) => {
  const data = await withdrawalService.getMyHistory(req.user.sub);
...
export const approveWithdrawalController = asyncHandler(async (req, res) => {
  const data = await withdrawalService.approve({ withdrawalId: req.params.id, adminId: req.user.sub });
