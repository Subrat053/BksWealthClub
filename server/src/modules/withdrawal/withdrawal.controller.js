...
export const createWithdrawalController = asyncHandler(async (req, res) => {
  const data = await withdrawalService.createRequest({
    userId: getAuthUserId(req),
    payload: req.body,
  });
  res
    .status(201)
    .json(new ApiResponse({ message: "Withdrawal request submitted", data }));
});

export const getWithdrawalHistoryController = asyncHandler(async (req, res) => {
  const data = await withdrawalService.getMyHistory(getAuthUserId(req));
  res.json(new ApiResponse({ message: "Withdrawal history fetched", data }));
});

export const getPendingWithdrawalController = asyncHandler(
  async (_req, res) => {
