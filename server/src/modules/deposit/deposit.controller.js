...
export const createDepositController = asyncHandler(async (req, res) => {
  const data = await depositService.createRequest({
    userId: req.auth.sub,
    payload: req.body,
  });
  res
    .status(201)
    .json(new ApiResponse({ message: "Deposit request created", data }));
});
...
export const getAllDepositsController = asyncHandler(async (_req, res) => {
  const data = await depositService.getAllRequestsForAdmin();
  res.json(new ApiResponse({ message: "All deposits fetched", data }));
});

export const approveDepositController = asyncHandler(async (req, res) => {
  const data = await depositService.approveRequest({
