...
export const createDepositController = asyncHandler(async (req, res) => {
  const data = await depositService.createRequest({ userId: req.user.sub, payload: req.body });
...
export const getMyDepositsController = asyncHandler(async (req, res) => {
  const data = await depositService.getMyHistory(req.user.sub);
...
export const approveDepositController = asyncHandler(async (req, res) => {
  const data = await depositService.approveRequest({ depositId: req.params.id, adminId: req.user.sub });
