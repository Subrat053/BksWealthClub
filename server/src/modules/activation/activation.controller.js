...
export const requestActivationController = asyncHandler(async (req, res) => {
  const data = await activationService.requestActivation({ userId: req.auth.sub });
...
export const executeActivationController = asyncHandler(async (req, res) => {
  const data = await activationService.executeActivationWorkflow({ userId: req.auth.sub });
