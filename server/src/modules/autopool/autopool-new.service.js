...
export const autoPoolNewService = {
   ...
   */
  createInitialAutoPoolEntriesAfterDeposit: async (userId, depositId, session = null) => {
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const rebirthRecord = new RebirthId({
        ownerUserId: userId,
        rebirthCode: rebirthCode,
        generation: 0,
        sourceEntryId: mainEntry._id,
      });
      await rebirthRecord.save({ session });

      const rebirthEntry = new AutoPoolEntry({
        ownerUserId: userId,
        displayId: rebirthCode,
        sourceType: "REBIRTH",
        rebirthLevel: 0,
        sourceEntryId: mainEntry._id,
        queueTimestamp: new Date(now.getTime() + i), // Slight offset to maintain FIFO
        status: "PENDING",
        metadata: { depositId: depositId.toString(), rebirthId: rebirthRecord._id.toString() }
      });
      await rebirthEntry.save({ session });

      rebirthRecord.autoPoolEntryId = rebirthEntry._id;
      await rebirthRecord.save({ session });
    }

    await autoPoolNewService.logAction("INITIAL_CREATION", null, userId, `Created Main + 2 Rebirth IDs for deposit ${depositId}`, null, { mainEntryId: mainEntry._id }, session);
    ...
    try {
      while (processedCount < MAX_PROCESS) {
        ...
        try {
          ...
          const pendingEntry = await AutoPoolEntry.findOne({ status: "PENDING" })
          ...
          const parent = await autoPoolNewService.findNextAvailableParent(session);

          if (parent) {
            await autoPoolNewService.placeEntryInMatrix(pendingEntry._id, parent._id, session);
          } else {
            ...
            const totalEntries = await AutoPoolEntry.countDocuments({ status: { $ne: "PENDING" } }).session(session);
            if (totalEntries === 0) {
              ...
              await autoPoolNewService.logAction("PLACE_ROOT", pendingEntry._id, pendingEntry.ownerUserId, "Placed as root node", null, null, session);
            } else {
              ...
              console.error("[AutoPool] No available parent found for pending entry", pendingEntry._id);
  ...
  placeEntryInMatrix: async (entryId, parentEntryId, session = null) => {
    ...
    if (parent.directChildrenCount >= 3) throw new Error("Parent is already full");
    ...
    await autoPoolNewService.logAction("PLACE_ENTRY", entry._id, entry.ownerUserId, `Placed under parent ${parent.displayId}`, null, { parentId: parent._id }, session);

    if (parent.status === "COMPLETED") {
      await autoPoolNewService.logAction("NODE_COMPLETED", parent._id, parent.ownerUserId, `Node ${parent.displayId} completed with 3 children`, null, null, session);
      await autoPoolNewService.createRebirthsAfterCompletion(parent._id, session);
    }
  ...
  createRebirthsAfterCompletion: async (completedEntryId, session = null) => {
    const completedEntry = await AutoPoolEntry.findById(completedEntryId).session(session);
    ...
    if (existingRebirths) {
      console.log(`[AutoPool] Rebirths already exist for completed entry ${completedEntryId}`);
      return;
    }

    const user = await User.findById(completedEntry.ownerUserId).session(session);
    const now = new Date();
    ...
    for (let i = 1; i <= 2; i++) {
      ...
      const rebirthEntry = new AutoPoolEntry({
        ...
        status: "PENDING",
        metadata: { rebirthId: rebirthRecord._id.toString(), fromCompletedEntry: completedEntryId.toString() }
      });
      await rebirthEntry.save({ session });
    ...
    await autoPoolNewService.logAction("REBIRTH_CREATED", completedEntryId, completedEntry.ownerUserId, `Created 2 rebirths for completion of ${completedEntry.displayId}`, null, null, session);
  },

  /**
   * Log an action in the Auto Pool system.
   */
  logAction: async (actionType, entryId, ownerUserId, message, beforeData, afterData, session = null) => {
