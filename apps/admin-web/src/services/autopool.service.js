...
export const autopoolService = {
  ...
  getStats: async () => {
    try {
      ...
      return response.data?.data || { totalEntries: 0, pendingEntries: 0, placedEntries: 0, completedEntries: 0, totalRebirths: 0 };
    } catch (error) {
      console.error("Error fetching autopool stats:", error);
      return { totalEntries: 0, pendingEntries: 0, placedEntries: 0, completedEntries: 0, totalRebirths: 0 };
    }
  },
